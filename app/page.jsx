...
export default function Home() {
  ...
  const formulaIcons = {
    contraintuitivo: '🔄', numero: '🔢', pregunta: '❓',
    historia: '📖', controversial: '⚡', lista: '📋',
  };
      ...
      <div style={{ background: 'white', borderBottom: '1px solid #E0E0E0', padding: '0 24px', position: 'sticky', top: 61, zIndex: 40 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', gap: 32 }}>
          {[
            ...
            return (
              <button key={tab.id} onClick={() => !disabled && setView(tab.id)}
                style={{
                  padding: '12px 0', fontSize: 12, fontWeight: 700, letterSpacing: 1,
          ...
          <div>
            ...
            <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, padding: 16, marginBottom: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    padding: '12px 24px', borderRadius: 8, border: 'none', fontWeight: 700, fontSize: 14,
                    background: loading ? '#999' : '#0A66C2', color: 'white', cursor: loading ? 'wait' : 'pointer',
                    whiteSpace: 'nowrap',
                  }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                {tags.map(tag => (
                  <button key={tag} onClick={() => { setKeyword(tag); handleAnalyze(tag); }}
            ...
            {loading && (
              <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, padding: 64, textAlign: 'center' }}>
                <div style={{ width: 48, height: 48, border: '4px solid #0A66C2', borderTopColor: 'transparent', borderRadius: '50%', margin: '0 auto 16px', animation: 'spin 1s linear infinite' }}></div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {(analysis.trends || []).map((t, i) => {
                    ...
                    const satColors = { low: { bg: '#DCFCE7', t: '#166534' }, medium: { bg: '#FEF9C3', t: '#854D0E' }, high: { bg: '#FEE2E2', t: '#991B1B' } };
                      ...
                      <div key={i} style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 12 }}>
                            <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 600, background: pc.bg, color: pc.text }}>{t.suggested_pillar}</span>
                            <span style={{ padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 600, background: sc.bg, color: sc.t }}>Sat: {t.saturation}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ marginBottom: 12 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
                            ...
                            <span style={{ fontWeight: 700, color: t.relevance_score >= 8 ? '#166534' : t.relevance_score >= 5 ? '#854D0E' : '#991B1B' }}>{t.relevance_score}/10</span>
                          </div>
                          <div style={{ height: 6, background: '#F3F4F6', borderRadius: 3, overflow: 'hidden' }}>
                            <div style={{
                              height: '100%', borderRadius: 3, width: `${t.relevance_score * 10}%`,
                        ...
                        <div style={{ background: '#EBF5FF', borderRadius: 8, padding: 12, marginBottom: 12 }}>
                          <p style={{ margin: 0, fontSize: 13, color: '#004182' }}>💡 <strong>Tu ángulo:</strong> {t.lucas_angle}</p>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          ...
                          <button onClick={() => handleGenerateHooks(t)}
                            style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#0A66C2', color: 'white', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                            Generar Hooks →
                          </button>
                        </div>
                      </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {analysis.unique_angles.map((a, i) => (
                        <div key={i} style={{ background: 'white', borderLeft: '4px solid #00C17A', borderRadius: 8, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                          ...
                          <span style={{ display: 'inline-block', marginTop: 8, padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 600, background: engagementColors[a.potential_virality]?.bg || '#F3F4F6', color: engagementColors[a.potential_virality]?.text || '#666' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {(hooksData.hooks || []).map((h, i) => {
                    ...
                    return (
                      <div key={i} style={{
                        background: 'white', border: isTop3 ? '2px solid #00C17A' : '1px solid #E0E0E0',
                        borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', position: 'relative',
                      }}>
                        {isTop3 && (
                          <span style={{ position: 'absolute', top: -10, right: 16, background: '#00C17A', color: 'white', padding: '2px 10px', borderRadius: 10, fontSize: 11, fontWeight: 700 }}>⭐ TOP 3</span>
                        )}
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => handleGeneratePost(h)}
                            style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#0A66C2', color: 'white', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                            Escribir Post →
                          </button>
                          <button onClick={() => copyText(h.text, `hook-${i}`)}
                            style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #E0E0E0', background: 'white', fontWeight: 600, fontSize: 13, cursor: 'pointer', color: '#666' }}>
              ...
              <div>
                ...
                <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                  ...
                  <div style={{ padding: '16px 20px', display: 'flex', gap: 12 }}>
                    <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #0A66C2, #00C17A)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'white', fontSize: 16, flexShrink: 0 }}>LV</div>
                    <div>
                  ...
                  <div style={{ padding: '0 20px 16px' }}>
                    {(() => {
                      ...
                      return (
                        <div>
                          ...
                          {truncated && (
                            <button onClick={() => setExpandedPost(true)}
                              style={{ background: 'none', border: 'none', color: '#666', fontWeight: 600, cursor: 'pointer', padding: 0, marginTop: 4, fontSize: 13 }}>
                              ...ver más
                            </button>
                          )}
                        </div>
                    ...
                    {/* Hashtags */}
                    <p style={{ color: '#0A66C2', fontSize: 14, marginTop: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', borderTop: '1px solid #E0E0E0', padding: '4px 8px' }}>
                    {['👍 Recomendar', '💬 Comentar', '🔄 Compartir', '📨 Enviar'].map(action => (
                      <button key={action} style={{ padding: '12px 0', background: 'none', border: 'none', color: '#666', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>{action}</button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
                  <button onClick={() => copyText(postData.post_body + '\n\n' + (postData.hashtags || []).join(' '), 'post')}
                    style={{ flex: 1, padding: '14px 24px', borderRadius: 8, border: 'none', background: '#0A66C2', color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
                    {copied === 'post' ? '✅ Post Copiado!' : '📋 Copiar Post Completo'}
                  </button>
                  <button onClick={() => copyText(postData.first_comment || '', 'comment')}
                    style={{ padding: '14px 20px', borderRadius: 8, border: '1px solid #E0E0E0', background: 'white', fontWeight: 600, fontSize: 13, cursor: 'pointer', color: '#666' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 120, alignSelf: 'start' }}>
            ...
            <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              <div style={{ padding: '0 16px 16px', textAlign: 'center', marginTop: -24 }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', border: '3px solid white', background: '#00C17A', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'white', fontSize: 18, margin: '0 auto 8px' }}>LV</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, borderTop: '1px solid #F3F4F6', paddingTop: 12 }}>
                  <div><p style={{ fontWeight: 700, color: '#0A66C2', margin: 0, fontSize: 14 }}>172</p><p style={{ fontSize: 9, color: '#999', margin: 0 }}>ORDENANZAS</p></div>
                  <div><p style={{ fontWeight: 700, color: '#0A66C2', margin: 0, fontSize: 14 }}>25K+</p><p style={{ fontSize: 9, color: '#999', margin: 0 }}>VECINOS</p></div>
                  <div><p style={{ fontWeight: 700, color: '#0A66C2', margin: 0, fontSize: 14 }}>189</p><p style={{ fontSize: 9, color: '#999', margin: 0 }}>PROYECTOS</p></div>
                </div>
              </div>
            </div>
              ...
              {[
                ...
                <div key={p.name} style={{ marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 3 }}>
                    <span>{p.name}</span><span style={{ fontWeight: 700 }}>{p.pct}%</span>
                  </div>
            ...
            <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
              ...
              {[
                ...
                <div key={s.day} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #F9FAFB', fontSize: 13 }}>
                  ...
                  <span><strong>{s.time}</strong> <span style={{ color: '#999' }}>/ {s.alt}</span></span>
                </div>
              ))}
            ...
            {history.length > 0 && (
              <div style={{ background: 'white', border: '1px solid #E0E0E0', borderRadius: 12, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
                ...
                {history.map((h, i) => (
                  <button key={i} onClick={() => handleAnalyze(h.query)}
                    style={{ display: 'block', width: '100%', textAlign: 'left', padding: '6px 0', background: 'none', border: 'none', borderBottom: '1px solid #F9FAFB', cursor: 'pointer', fontSize: 13, color: '#0A66C2' }}>
                    {h.query}
                  </button>
                ))}
              </div>
            ...
            <div style={{ background: '#0B1931', borderRadius: 12, padding: 16, color: 'white' }}>
              ...
              {[
                'Engagement 1ra hora: 30%',
                'Tiempo lectura: 25% del score',
                'Comentarios +15 palabras: 20%',
                'No editar en 60 min post-publicar',
                'Máximo 5 hashtags',
              ].map((tip, i) => (
